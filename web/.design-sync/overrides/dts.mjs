// forked from design-sync lib/dts.mjs - strictNullChecks so `| null` survives in prop types
...
function projectFor(pkgDir, typesRoot) {
  ...
  const project = new Project({
    ...
    compilerOptions: {
      ...
      skipLibCheck: true,
      strict: false,
      // Forked line. With strictNullChecks off — which `strict: false` implies
      // — the checker collapses `number | null` to `number`, and this DS uses
      // null as a real state in a dozen contracts (no baseline, no report, a
      // gate that is off). Emitting `value: number` there tells the design
      // agent the opposite of the truth, so the null side is kept.
