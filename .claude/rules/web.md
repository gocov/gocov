...
- What stays a server route is a browser navigation or a machine endpoint: OAuth, `/logout`, the consent start (`/workspace-connect/{forge}/{prefix...}`), `/github/setup`, the raw profile download, badge, upload API. Their redirects land on app routes and carry codes, never prose (`?connect=`, `?error=connect_failed|connect_denied|connect_owners_only`); the app writes the sentences (`web/src/lib/connect.ts`, and each page's code table for `lib/notice.ts`, which drops any value it has no sentence for).
