...
- What stays a server route is a browser navigation or a machine endpoint: OAuth, `/logout`, the consent start (`/workspaces/{forge}/{prefix}/connect`), `/github/setup`, the raw profile download, badge, upload API. Their redirects land on app routes and carry codes, never prose (`?connect=`, `?error=connect_failed`); the app writes the sentences (`web/src/lib/connect.ts`, `lib/notice.ts`).
