...
# The docs tell users which release to download. check-pins.sh keeps those
# pins in step with each other; only here, after the release exists, can
# they be held to the release itself. The web UI's setup screen is not a
# copy: it writes its snippet from hosted.PinnedCLIVersion, which
# hosted.go below carries.
docs_stale=""
for path in docs/gitlab-ci.md docs/ci-other.md docs/self-hosting.md internal/hosted/hosted.go; do
  ...
  for v in $(echo "$body" | grep -Eo -e 'releases/download/v[0-9]+\.[0-9]+\.[0-9]+' -e 'ver=v[0-9]+\.[0-9]+\.[0-9]+' -e 'gocov-server:v[0-9]+\.[0-9]+\.[0-9]+' \
    -e 'PinnedCLIVersion = "v[0-9]+\.[0-9]+\.[0-9]+' |
