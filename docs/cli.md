...
1. **OIDC.** If the CI job can mint an OIDC identity token (GitHub Actions with `permissions: id-token: write`),
   the CLI requests one scoped to the gocov server's audience and sends it in place of the token. The server
   verifies the forge's signature and the repository claim, so the upload is fully verified — not marked
   unverified. This is the recommended setup for a repo's own `push` and same-repo PR builds; see
